...
// ─── Normalise risk_level (supporte français, anglais, accents) ───────────────
...
export default function DashboardStats({ lang, profile, negotiations }) {
  ...
  const riskTotal = Math.max(riskCounts.high + riskCounts.medium + riskCounts.low, 1);
  ...
  const stats = [
    {
      ...
      bg: 'bg-shield-green/10',
      border: 'border-shield-green/20',
    },
    {
      icon: History,
            ...
            <div className="bg-shield-card border border-shield-border rounded-xl p-5">
              ...
              <div className="grid grid-cols-2 gap-2">
                {topCities.map(([city, count]) => (
                  <div
                    key={city}
                    className="flex items-center justify-between bg-shield-dark border border-shield-border rounded-lg px-3 py-2"
                  >
